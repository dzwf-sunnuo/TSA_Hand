...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void Set_Wrist_Speed(uint8_t Mode,uint8_t speed)
{
...
//        case 8:
//            break;
        default:  break;// 所有 case 都不匹配时执行
    }
	}
	else if(Mode == CONSTAND_SPEED){
		switch (speed) {
        case 3:
				break;
...
//			printf("111%d\n",*key_num);
		if(*key_num == IR_8){//8号动作复位  手腕复位
			*key_num  = IR_Empty;//转移到无效的动作指令
				...
				cycle_time = 3000;
				cycle_time_R = 3000;
				uint16_t stop[5] = {0x0064,0x0064,0x0064,0x0064,0x0101};
...
//			Reg[14] = 0x0101;
		...
		NUMActionCycle=NUMActionCycle+1;		  
			...
			if(up_count <7){
				...
				up_count = 6;}
     Set_Wrist_Speed(VARIABLE_SPEED,up_count);			
