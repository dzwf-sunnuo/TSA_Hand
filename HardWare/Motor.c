...
const Motor_HW_Config motor_hw_map[Motor_Num] = {
    ...
    {&htim1, TIM_CHANNEL_2, TIM_CHANNEL_1, &htim4}, // 电机 2
...
void Motor_Init(void)
{
    ...
    HAL_TIM_Base_Start_IT(&htim6); // 开启 10ms 控制律计算定时器
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance == htim7.Instance) {
        if (modbus.timrun != 0) {
            modbus.timout++;
            if (modbus.timout >= 8) { // 8ms 帧结束判定
                modbus.timrun = 0;
                modbus.reflag = 1;
            }
        }
        modbus.Host_Sendtime++;
        if (modbus.Host_Sendtime > 1000) { // 1s 定时标志
        ...
        for (int i = 0; i < Motor_Num; i++) {
            ...
            switch (mode) {
                ...
                case 5: // 模式5：关节角度插值控制 (接收 Reg[i] 为目标关节物理角度)
                {
                    ...
                    // 安全保护：若电机转动超过 50 圈则强制停止以防拉断钢丝
                    if (motor[i].CurrentAngle < -50.0f * Real_OneTurn || motor[i].CurrentAngle > 50.0f * Real_OneTurn) motor[i].TargetSpeed = 0.0f;
                    break;
                }
                default:
                    motor[i].TargetSpeed = 0.0f;
