...
/* 全局控制变量 */
// wrist_enabled / wrist_enabled_constand 已消除，改为队列驱动的 Task_WristMotion
// 启停通过 WristMotion_Start/Stop → Queue_WristCmd → 任务本地 run_mode 实现
    ...
    {    
        switch (data1)
        {
            case 0x00: // 停止
                ...
                break;
            case 0x01: // 开启摆动 (通过 wrapper 同步信号量)
    ...
    else if(data0 == 0x11) // 旋转运动控制
    {
        switch (data1)
        {
            case 0x00: // 停止
                ...
                vel_move(0x04,0x00,0x0B,0xF0);
                break;
            case 0x01: // 开启旋转 (通过 wrapper 同步信号量)
